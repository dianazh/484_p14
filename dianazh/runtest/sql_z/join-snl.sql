...
-- Operator LT
-- Join on int
SELECT DA.serial, DA.ikey, DB.ikey, DA.dkey, DB.dkey, DA.filler, DB.filler FROM DA, DB WHERE DA.ikey < DB.ikey;
-- Join on double
SELECT DA.serial, DA.ikey, DB.ikey, DA.dkey, DB.dkey, DA.filler, DB.filler FROM DA, DB WHERE DA.dkey < DB.dkey;
-- Join on string
SELECT DA.serial, DA.ikey, DB.ikey, DA.dkey, DB.dkey, DA.filler, DB.filler FROM DA, DB WHERE DA.filler < DB.filler;

-- Project 5 attrs, filter attr doesn't appear in projection
-- Operator LTE
-- Join on int
SELECT DA.serial, DA.dkey, DB.dkey, DA.filler, DB.filler  FROM DA, DB WHERE DA.ikey <= DB.ikey;
-- Join on double
SELECT DA.serial, DA.ikey, DB.ikey, DA.filler, DB.filler FROM DA, DB WHERE DA.dkey <= DB.dkey;
-- Join on string
SELECT DA.serial, DA.ikey, DB.ikey, DA.dkey, DB.dkey FROM DA, DB WHERE DA.filler <= DB.filler;

-- Project 3 attrs, only one side's info appears
-- Operator GTE
-- Join on int
SELECT DA.ikey, DA.dkey, DA.filler FROM DA, DB WHERE DA.ikey >= DB.ikey;
-- Join on double
SELECT DA.dkey, DA.dkey, DA.filler FROM DA, DB WHERE DA.dkey >= DB.dkey;
-- Join on string
SELECT DA.filler, DA.dkey, DA.filler FROM DA, DB WHERE DA.filler >= DB.filler;

-- Project 3 attrs, only one side's info appears
-- Operator GT
-- Join on int
SELECT DB.ikey, DB.dkey, DB.filler FROM DA, DB WHERE DA.ikey > DB.ikey;
-- Join on double
SELECT DB.ikey, DB.dkey, DB.filler FROM DA, DB WHERE DA.dkey > DB.dkey;
-- Join on string
SELECT DB.ikey, DB.dkey, DB.filler FROM DA, DB WHERE DA.filler > DB.filler;

-- Project 6 random attrs
-- Operator NE
-- Join on int
SELECT DA.serial, DA.dkey, DB.dkey, DA.filler, DB.filler, DA.ikey  FROM DA, DB WHERE DA.ikey <> DB.ikey;
-- Join on double
SELECT DA.serial, DA.ikey, DB.ikey, DA.filler, DB.filler, DA.dkey FROM DA, DB WHERE DA.dkey <> DB.dkey;
-- Join on string
SELECT DA.serial, DA.ikey, DB.ikey, DA.dkey, DB.dkey, DA.filler FROM DA, DB WHERE DA.filler <> DB.filler;
